...
import { calculateDose as calculateDoseLib, generateChartData } from '@/lib/pharmacology/calculations';
...
export function useCalculatorState() {
  ...
  const calculateDose = useCallback((): any => {
    const customExponent = state.scalingExponent === 'custom' ?
    ...
    const gfr = ((140 - state.patientAge) * state.targetWeight) / (72 * state.patientCreatinine) * sexFactor;
    ...
    const result = calculateDoseLib(
      ...
      state.targetAnimal,
      {
        ...
        bioavailability: state.bioavailabilityMethod === 'iv' ? 100 :
                        state.bioavailabilityMethod === 'oral' ? 50 :
                        state.bioavailabilityMethod === 'other' ? 75 :
        ...
        kidneyFunction: state.kidneyFunctionMethod === 'none' ? 100 :
                       state.kidneyFunctionMethod === 'manual' ? state.kidneyFunction :
