...
export function validateWeight(weight: number): ValidationResult {
  ...
  if (isNaN(weight) || !isFinite(weight)) {
    ...
    warnings.push('Very small weight detected. Please verify the value is correct');
  } else if (weight > 1000) {
    warnings.push('Very large weight detected. Please verify the value is correct');
  }
...
export function validateDose(dose: number): ValidationResult {
  ...
  if (isNaN(dose) || !isFinite(dose)) {
    ...
    warnings.push('Very small dose detected. Consider using micrograms (mcg) instead');
  } else if (dose > 10000) {
    warnings.push('Very large dose detected. Please verify the value is correct');
  }
...
export function validateProteinBinding(value: number): ValidationResult {
  ...
  if (isNaN(value) || !isFinite(value)) {
    ...
    errors.push(`Protein binding cannot exceed ${VALIDATION_LIMITS.proteinBinding.max}%`);
  } else if (value > 95) {
    warnings.push('Very high protein binding (>95%) detected. This will significantly affect the dose');
  }
...
export function validateBioavailability(value: number): ValidationResult {
  ...
  if (isNaN(value) || !isFinite(value)) {
    ...
    errors.push(`Bioavailability must be at least ${VALIDATION_LIMITS.bioavailability.min}%`);
  } else if (value > VALIDATION_LIMITS.bioavailability.max) {
    errors.push(`Bioavailability cannot exceed ${VALIDATION_LIMITS.bioavailability.max}%`);
  } else if (value < 10) {
    warnings.push('Very low bioavailability (<10%) detected. This will significantly increase the dose');
  }
...
export function validateCreatinine(value: number): ValidationResult {
  ...
  if (isNaN(value) || !isFinite(value)) {
    ...
    errors.push(`Creatinine must be at least ${VALIDATION_LIMITS.creatinine.min} mg/dL`);
  } else if (value > VALIDATION_LIMITS.creatinine.max) {
    errors.push(`Creatinine must not exceed ${VALIDATION_LIMITS.creatinine.max} mg/dL`);
  } else if (value > 2) {
    warnings.push('Elevated creatinine detected. Consider kidney function adjustment');
  }
...
export function validateAge(value: number): ValidationResult {
  ...
  if (isNaN(value) || !isFinite(value)) {
    ...
    warnings.push('Advanced age detected. Consider additional dose adjustments');
  } else if (value < 18) {
...
export function validateScalingExponent(value: number): ValidationResult {
  ...
  if (isNaN(value) || !isFinite(value)) {
    ...
    errors.push(`Scaling exponent must be at least ${VALIDATION_LIMITS.scalingExponent.min}`);
  } else if (value > VALIDATION_LIMITS.scalingExponent.max) {
    errors.push(`Scaling exponent must not exceed ${VALIDATION_LIMITS.scalingExponent.max}`);
  } else if (value < 0.5 || value > 1.0) {
    warnings.push('Unusual scaling exponent. Standard values are typically between 0.67 and 0.75');
  }
...
export function validateCalculationInputs(
  ...
  errors.push(...targetWeightValidation.errors.map(e => `Target weight: ${e}`));
  errors.push(...doseValidation.errors);

  warnings.push(...baseWeightValidation.warnings.map(w => `Source weight: ${w}`));
  warnings.push(...targetWeightValidation.warnings.map(w => `Target weight: ${w}`));
  ...
  if (baseWeight === targetWeight && (method === 'brainWeight' || method === 'lifeSpan' || method === 'hepaticFlow')) {
    warnings.push('Source and target weights are equal. Some scaling methods may produce undefined results');
  }
